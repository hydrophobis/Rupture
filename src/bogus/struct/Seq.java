...
public class Seq<T> implements Iterable<T>, Eachable<T>{
    ...
    /**
     * Returns an index of last occurrence of value in array or -1 if no such value exists. Sebogush is started from the end of an
     * array.
     * @param value May be null.
     * @param identity If true, == comparison will be used. If false, .equals() comparison will be used.
     ...
     * array must have a size greater than 0, or a {@link BogusRuntimeException} will be thrown.
    ...
    public T selectRanked(Comparator<T> comparator, int kthLowest){
        if(kthLowest < 1){
            throw new BogusRuntimeException("nth_lowest must be greater than 0, 1 = first, 2 = second...");
        }
    ...
    public static class SeqIterable<T> implements Iterable<T>{
        ...
        private class SeqIterator implements Iterator<T>{
            ...
            @Override
            public void remove(){
                if(!allowRemove) throw new BogusRuntimeException("Remove not allowed.");
                index--;
                array.remove(index);
            }
        }
