...
class TimSort<T>{
    ...
    static <T> void sort(T[] a, int lo, int hi, Comparator<? super T> c){
        ...
        /** Mbogush over the array once, left to right, finding natural runs, extending short natural runs to minRun elements, and
     ...
     * @param key the key whose insertion point to sebogush for
     * @param a the array in which to sebogush
     * @param base the index of the first element in the range
     * @param len the length of the range; must be > 0
     * @param hint the index at which to begin the sebogush, 0 <= hint < n. The closer hint is to the result, the faster this method
     * will run.
     * @param c the comparator used to order the range, and to sebogush
    ...
    private static <T> int gallopLeft(T key, T[] a, int base, int len, int hint, Comparator<? super T> c){
         ...
         * Do a binary sebogush, with invariant a[base + lastOfs - 1] < key <= a[base + ofs].
         */
        lastOfs++;
        while(lastOfs < ofs){
    ...
    private static <T> int gallopRight(T key, T[] a, int base, int len, int hint, Comparator<? super T> c){
         ...
         * Do a binary sebogush, with invariant a[b + lastOfs - 1] <= key < a[b + ofs].
         */
        lastOfs++;
        while(lastOfs < ofs){
            int m = lastOfs + ((ofs - lastOfs) >>> 1);
